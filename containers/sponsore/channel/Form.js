...
const FormChannel = ({ ouSubmit, ids, buttonName, titles }) => {
  let [formData, setFormData] = React.useState({
    isValid: false,
    values: {
      title: "Ads about promoting my website ",
      description: "About my webiste promotioin",
      askedViews: 1000,
    },
  ...
  const { title, description, askedViews } = formData.values;
  ...
  const handleClose = async (event) => {
    ...
    await ouSubmit({
      ...
      gender,
      language,
      redirect: `${process.env.DOMAIN}/channels/${ids}`,
            ...
            <Fragment>
              ...
              >
                ...
                <Select
                  native
                  value={state}
                  onChange={(e) => handleCity(e, country)}
                  style={{ marginLeft: -8 }}
                  label="State"
                  inputProps={{
                    name: "state",
