...
import ChannelPublisherId from "../../containers/channelPublisherId";

const ChannelById = ({ sponsoredChannel, channelId }) => {
  const head = () => (
    <Head>
  ...
  return (
    <Fragment>
      ...
      <ChannelPublisherId sponsoredChannel={sponsoredChannel} />
    </Fragment>
  );
};

ChannelById.getInitialProps = async ({ query, req }) => {
  let token = await parseCookies(req).token;
  const res = await axios.get(
    `${process.env.SERVER_API}/sponsorechannelbyid/${query.channel}`,
    {
      headers: {
